...
- [ ] T1: Schema — `FillUp` model + migration (AC6, AC8)
  - [ ] T1a: Add `FillUp` model to `packages/db/prisma/schema.prisma` (see Dev Notes)
  - [ ] T1b: Add `fillUps FillUp[]` relation to `User` and `Vehicle` models; add optional `fillUps FillUp[]` to `Station`
  - [ ] T1c: Create migration `packages/db/prisma/migrations/<timestamp>_add_fill_ups/migration.sql`

- [ ] T2: `FillupOcrService` — synchronous Claude Haiku OCR (AC3, AC9, AC10)
  - [ ] T2a: Create `apps/api/src/fillup/fillup-ocr.service.ts`
  - [ ] T2b: Implement `extractFromPumpMeter(imageBuffer, mediaType)` — calls Claude Haiku with PUMP_METER_OCR_PROMPT; returns `FillupOcrResult`; returns `{ confidence: 0 }` (all nulls) on parse failure or low confidence
  - [ ] T2c: Track Haiku spend for pump meter OCR in existing `OcrSpendService.recordSpend()` — same daily budget

- [ ] T3: `FillupService` — business logic (AC6, AC7, AC8)
  - [ ] T3a: Create `apps/api/src/fillup/fillup.service.ts`
  - [ ] T3b: Implement `createFillup(userId, dto)`:
    - Validate vehicle belongs to user (throw 403 if not)
    - GPS station match: call `StationService.findNearest(lat, lng, 200)` — nullable result
    - Fetch benchmark: call `RegionalBenchmarkService.getLatestForStation(stationId, fuelType)` — only if station matched
    - Write `FillUp` record
    - If station matched: write `PriceHistory` record (`source: 'community'`) and clear `StationFuelStaleness` for (station_id × fuel_type)
    - Set `vehicle.is_locked = true` (upsert-safe: only update if currently false)
  - [ ] T3c: Implement `listFillups(userId, vehicleId?, page, limit)` — paginated, newest first

- [ ] T4: `FillupController` — API endpoints (AC3, AC6, AC9, AC10, AC11)
  - [ ] T4a: Create `apps/api/src/fillup/fillup.controller.ts` — routes under `v1/me/fillups`
  - [ ] T4b: `POST /v1/me/fillups/ocr` — accepts `multipart/form-data` with `photo` field; 10s timeout guard; returns `FillupOcrResult`; always 200 (confidence=0 means fallback to manual)
  - [ ] T4c: `POST /v1/me/fillups` — create fill-up; accepts JSON body with confirmed values; returns created FillUp + station match info for celebration screen
  - [ ] T4d: `GET /v1/me/fillups` — list fill-ups, supports `?vehicleId=` filter, `?page=` / `?limit=`

- [ ] T5: `FillupModule` + app registration
  - [ ] T5a: Create `apps/api/src/fillup/fillup.module.ts` — registers services, controller; imports `RegionalBenchmarkModule`, `PrismaModule`, `StationModule`
  - [ ] T5b: Import `FillupModule` in `apps/api/src/app.module.ts`

...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!station) return { title: 'Station — Litro' };
  const pb95 = station.price?.prices['PB_95'];
  return {
    title: `${station.name} — fuel prices | Litro`,
    description: `Current fuel prices at ${station.name}${station.address ? `, ${station.address}` : ''}.${pb95 ? ` Petrol 95: ${pb95.toFixed(2)} PLN/l.` : ''}`,
  };
}

const FUEL_ORDER = ['PB_95', 'PB_98', 'ON', 'ON_PREMIUM', 'LPG'];

export default async function StationDetailEnPage({ params }: Props) {
  const { id } = await params;
  const station = await fetchStationWithPrice(id);
  if (!station) notFound();
                ...
                <div className="border border-gray-200 rounded-xl overflow-hidden">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="text-left px-4 py-2.5 font-medium text-gray-600">Fuel type</th>
                        <th className="text-right px-4 py-2.5 font-medium text-gray-600">PLN/l</th>
                  ...
                  {station.price.updatedAt && (
                    <div className="px-4 py-2 bg-gray-50 border-t border-gray-100 text-xs text-gray-400">
                      {t.station.lastUpdated}: {new Date(station.price.updatedAt).toLocaleDateString('en-GB')}
                    </div>
                  )}
                </div>
              ) : (
